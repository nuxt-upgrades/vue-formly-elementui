...
export default {
  ...
  created() {
    ...
    if (
      this.inputType === 'checkbox' && (this.model[this.field.key].constructor !== Array
      || !this.model[this.field.key].constructor)
    ) {
  ...
  render(h) {
    ...
    if (this.to.label) {
      children.push(h('label', this.to.label));
    }

    // create each option
    if ('options' in this.field) {
      this.field.options.forEach((option) => {
        const optionLabel = Object.prototype.hasOwnProperty.call(option, 'label') ? option.label : option;
        const optionVal = Object.prototype.hasOwnProperty.call(option, 'value') ? option.value : option;
        const tester = this.model[this.field.key];
        const optionChecked = isArray ? tester.indexOf(optionVal) > -1 : tester === optionVal;
        children.push(
                ...
                change(event) {
                  ...
                  if (isArray) {
                    if (isChecked) {
                      ...
                      if (valueIdx > -1) {
                        self.model[self.field.key].splice(valueIdx, 1);
                      }
                    }
                  } else {
